...
export class ChipSet extends LitElement {
  ...
  private updateTabIndices() {
    ...
    const {chips} = this;
    let firstFocusableChip: Chip|undefined;
    ...
    for (const chip of chips) {
      ...
      if (chipIsFocused && isChipFocusable) {
        chip.tabIndex = 0;
        hasFocusedChip = true;
        firstFocusableChip = chip;
        continue;
      }

      if (isChipFocusable && !firstFocusableChip) {
        firstFocusableChip = chip;
      }
    ...
    if (!hasFocusedChip && firstFocusableChip) {
      // None of the chips are currently focused. Set the first focusable chip
      // as the tab target.
      firstFocusableChip.tabIndex = 0;
    }
  }
}
