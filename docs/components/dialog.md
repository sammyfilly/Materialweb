...
`type` | `string` | `undefined` | The type of dialog for accessibility. Set this to `alert` to announce a<br>dialog as an alert dialog.
`open` | `boolean` | `undefined` | Opens the dialog when set to `true` and closes it when set to `false`.
...
--- | --- | --- | --- | ---
`open` | `undefined` | No | No | Dispatched when the dialog is opening before any animations.
`opened` | `undefined` | No | No | Dispatched when the dialog has opened after any animations.
`close` | `undefined` | No | No | Dispatched when the dialog is closing before any animations.
`closed` | `undefined` | No | No | Dispatched when the dialog has closed after any animations.
`cancel` | `undefined` | No | No | Dispatched when the dialog has been canceled by clicking on the<br>scrim or pressing Escape.
